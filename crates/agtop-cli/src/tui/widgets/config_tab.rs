...
//! virtual list [clients..., columns...]. Keys & mouse both route
...
pub fn render(frame: &mut Frame<'_>, area: Rect, app: &App, out: ConfigRenderOut<'_>) {
    ...
    let columns_block_height = area
        .height
        .saturating_sub(clients_block_height)
        .max(columns_min);

    let rows_layout = Layout::default()
        .direction(Direction::Vertical)
        .constraints([
            Constraint::Length(clients_block_height),
            Constraint::Length(columns_block_height),
        ])
        .split(area);

    render_clients(frame, rows_layout[0], app, out.client_rows);
    render_columns(frame, rows_layout[1], app, out.column_rows);
}

fn render_clients(frame: &mut Frame<'_>, area: Rect, app: &App, out: &mut Vec<(Rect, usize)>) {
    let block = Block::default()
        .borders(Borders::ALL)
        .title(" Clients — Space/click: toggle ");

    let clients = &app.column_config().clients;
    ...
    if app.config_section_at(cursor) == ConfigSection::Clients {
    ...
    for (i, _) in clients.iter().enumerate() {
        ...
        out.push((rect, i)); // virtual idx == local idx for clients
    }
...
fn render_columns(frame: &mut Frame<'_>, area: Rect, app: &App, out: &mut Vec<(Rect, usize)>) {
    ...
    let cursor = app.config_cursor();
    let n_clients = col_cfg.clients.len();
    ...
    let rows: Vec<Row> = col_cfg
        ...
        .map(|(i, entry)| {
            ...
            // Virtual index for this row is (n_clients + i).
            let row_style = if (n_clients + i) == cursor {
                th::SELECTED
            } else {
                ratatui::style::Style::new()
            };
