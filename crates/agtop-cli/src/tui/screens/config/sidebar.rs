...
const SECTIONS: [(ConfigSection, &str, Icon); 7] = [
    ...
    (ConfigSection::DataSources, "Data sources", Icon::DatabaseOutline),
    (ConfigSection::About,       "About",        Icon::InformationOutline),
];

pub fn render(frame: &mut Frame<'_>, area: Rect, current: ConfigSection, nerd_font: bool, theme: &Theme) {
    ...
    let lines: Vec<Line> = SECTIONS.iter().map(|(sec, label, icon)| {
        let icon_str = icon.render(nerd_font);
        let prefix = if !icon_str.is_empty() { format!("  {icon_str}  ") } else { "  ".to_string() };
        if *sec == current {
            Line::from(vec![
                Span::styled(prefix, Style::default().fg(theme.accent_primary)),
                Span::styled(format!("‹ {label} ›"), Style::default().fg(theme.accent_primary).add_modifier(Modifier::BOLD)),
            ])
        } else {
            Line::from(vec![
...
mod tests {
    ...
    fn renders_without_panic() {
        ...
        term.draw(|f| render(f, Rect::new(0, 0, 30, 20), ConfigSection::Appearance, false, &theme)).unwrap();
    }
}
