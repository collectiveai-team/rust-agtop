...
const ALL_CLIENTS: [ClientKind; 7] = [
    ClientKind::Claude, ClientKind::Codex, ClientKind::GeminiCli,
    ClientKind::Copilot, ClientKind::Cursor, ClientKind::Antigravity, ClientKind::OpenCode,
];

pub fn render(frame: &mut Frame<'_>, area: Rect, m: &ClientsModel, theme: &Theme) {
    let title = Line::from(Span::styled("Clients", Style::default().fg(theme.fg_emphasis).add_modifier(Modifier::BOLD)));
    ...
    let rows: Vec<(ClientKind, bool, String)> = ALL_CLIENTS.iter().map(|&kind| {
        let enabled = m.enabled.get(&kind).copied().unwrap_or(true);
        let path = m.custom_paths.get(&kind).cloned().unwrap_or_else(|| "(default)".into());
        (kind, enabled, path)
    }).collect();
    ...
    for (kind, enabled, path) in &rows {
        lines.push(Line::from(vec![
            Span::styled(format!("  {:<14}", kind.as_str()), Style::default().fg(theme.fg_default)),
