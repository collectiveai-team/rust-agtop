...
impl AggregationTable {
    pub fn render(&self, frame: &mut Frame<'_>, area: Rect, theme: &Theme) {
        let header = Row::new(vec!["GROUP", "SESSIONS", "TOKENS", "COST", "AVG DUR", "LAST ACTIVE", "ACTIVITY"])
            .style(Style::default().fg(theme.fg_emphasis).add_modifier(Modifier::BOLD));
        ...
        if !self.groups.is_empty() {
            ...
            rows.push(Row::new(vec![
                ...
                Cell::from(total_cost.map(|c| format!("${c:.2}")).unwrap_or_else(|| "—".into())),
                Cell::from(""),
                Cell::from(""),
                Cell::from(""),
            ])
    ...
    fn row_for<'a>(g: &'a AggregateGroup, theme: &Theme) -> Row<'a> {
        ...
        let cost = g.total_cost.map(|c| format!("${c:.2}")).unwrap_or_else(|| "—".into());
        let last = g.last_active.map(format_relative).unwrap_or_default();
        let dur = format!("{}m {}s", g.avg_duration_secs / 60, g.avg_duration_secs % 60);
        Row::new(vec![
