...
use agtop_cli::tui::widgets::tab_bar;
...
fn tab_bar_dashboard_active_120_cols() {
    ...
    let buf = render_to_buffer(120, 1, |f| {
        tab_bar::render(f, Rect::new(0, 0, 120, 1), ScreenId::Dashboard, "0.4.0", &theme);
    });
...
fn tab_bar_config_active_140_cols() {
    ...
    let buf = render_to_buffer(140, 1, |f| {
        tab_bar::render(f, Rect::new(0, 0, 140, 1), ScreenId::Config, "0.4.0", &theme);
    });
